...
df = data.frame(
  ...
  mutate(
    trt.pi = exp(z1 + z2 + z3) / (1 + exp(z1 + z2 + z3))
  ) %>%
  rowwise() %>%
  mutate(
...
# make design matrix
make_design_B = function(t.basis, z1.basis, z2.basis, z3.basis) {
  x.basis = cbind(t.basis, z1.basis, z2.basis, z3.basis)
  ...
  compute_Gh_batch = function(batch_rows) {
    ...
    for (rr in batch_rows) {
      ...
      survival0.id = 1 - phare(
        df.id$t_k,
        c(df.id[1, c("z1", "z2", "z3")], m.trt0),
        surv_c.list[[cf_fold]]
      )
      
      survival1.id = 1 - phare(
        df.id$t_k,
        c(df.id[1, c("z1", "z2", "z3")], m.trt1),
        surv_c.list[[cf_fold]]
      )
...
z1.basis.obj.oracle = splines::ns(
  df$z1,
  df = oracle_degree_of_freedom
)
...
z3.basis.obj.oracle = splines::ns(
  df$z3,
  df = oracle_degree_of_freedom
)
...
new_basis_oracle = make_design_B(
  t.basis = newt.basis,
  z1.basis = newz1.basis,
  z2.basis = newz2.basis,
  z3.basis = newz3.basis
